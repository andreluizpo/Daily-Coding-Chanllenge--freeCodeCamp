/*
freeCodeCamp - Daily Coding Challenges (10/20/2025) 
...
function calculateTips(mealPrice, customTip) {
    const customTipPercent = Number(customTip.replace("%", "")) / 100;
    const percentages = [0.15, 0.2, customTipPercent];
    const formattedMealPrice = Number(mealPrice.slice(1));

    return percentages.map(
        (percentage) => `$${(formattedMealPrice * percentage).toFixed(2)}`
    );
}
