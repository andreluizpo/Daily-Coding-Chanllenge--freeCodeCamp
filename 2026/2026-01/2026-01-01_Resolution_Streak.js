/*
freeCodeCamp - Daily Coding Challenges (01/01/2026) 
...
function resolutionStreak(days) {
  ...
  for (const day of days) {
    ...
    const numberSteps = day[0];
    const screenTime = day[1];
    const pagesRead = day[2];

    if (numberSteps >= 10000 && screenTime <= 120 && pagesRead >= 5) {
      sequenceDays++;
    } else {
      return `Resolution failed on day ${currentDay}: ${sequenceDays} day streak.`;
    }
  }

  return `Resolution on track: ${sequenceDays} day streak.`;
}
